...
//!
//! Per the crate convention (see [`crate::types`]), atilla is synchronous and
...
//! guards against thrown JS errors; atilla's tool/hook closures return values
...
fn stream_assistant_response(
    ...
    // Resolve API key (important for expiring tokens). atilla-ai's StreamOptions
    ...
    for event in response.events {
        match &event {
            atilla_ai::AssistantMessageEvent::Start { partial } => {
            ...
            atilla_ai::AssistantMessageEvent::Done { .. }
            | atilla_ai::AssistantMessageEvent::Error { .. } => {
...
/// error on failure. atilla-ai does not yet port TypeBox, and [`AgentTool`]
...
///
/// pi's `try/catch` maps a thrown execute error to an error result; atilla's
...
/// atilla's hook returns `Option<AfterToolCallResult>` and cannot throw, so the
...
fn event_partial(event: &atilla_ai::AssistantMessageEvent) -> Option<&AssistantMessage> {
    use atilla_ai::AssistantMessageEvent as E;
    match event {
        E::Start { partial }
        | E::TextStart { partial, .. }
        | E::TextDelta { partial, .. }
