...
fn mock_model() -> atilla_ai::Model {
    use atilla_ai::providers::faux::{FauxProvider, RegisterFauxProviderOptions};
...
fn should_not_execute_tool_calls_from_a_length_truncated_message() {
    ...
    let stream_fn: StreamFn = Arc::new(move |_model, _ctx, _opts, _signal| {
        ...
        let message = if i == 0 {
            assistant_message(
                vec![tool_call_block("echo", "tool-1", json!({ "value": "hel" }))],
                StopReason::Length,
            )
        } else {
            assistant_message(vec![text_block("done")], StopReason::Stop)
...
// ---------------------------------------------------------------------------
// Supplementary — abort branches under-exercised by the TS suite
// ---------------------------------------------------------------------------
