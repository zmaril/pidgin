...
//! `@earendil-works/pi-ai`; here those map onto the ported [`atilla_ai`] types,
...
//! async-iterable produced by `StreamFn`). atilla-ai re-presents streaming
//! **eagerly**: [`atilla_ai::seams::Provider::stream`] returns a
...
//!
//! - [`StreamFn`] mirrors [`atilla_ai::seams::Provider::stream`]'s signature and
//!   returns a [`StreamResult`] directly (no future). An `atilla-ai` provider
...
///
/// pi's `StreamFn` returns an async `AssistantMessageEventStream`; in atilla the
/// provider seam returns a [`StreamResult`] eagerly, so this closure mirrors
/// [`atilla_ai::seams::Provider::stream`] and returns that result directly. The
...
/// The port reuses [`atilla_ai::ModelThinkingLevel`] rather than redefining it.
...
/// [`atilla_ai::ContentBlock::ToolCall`] byte-for-byte.
...
/// `types.ts:145`).
///
/// pi's `AgentLoopConfig extends SimpleStreamOptions`. atilla-ai ports a subset
...
pub struct AgentLoopConfig {
    /// Inherited [`SimpleStreamOptions`](atilla_ai::StreamOptions) subset.
