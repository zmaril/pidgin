//! Tests the additive bridge `impl compaction::Models for atilla_ai::Models`:
//! compaction can now run against atilla-ai's real `completeSimple` wrapper,
...
/// Build an `atilla_ai::Models` whose single provider streams through a
...
fn faux_models(responses: Vec<atilla_ai::AssistantMessage>) -> (atilla_ai::Models, Model) {
...
fn text_message(text: &str) -> atilla_ai::AssistantMessage {
...
/// The bridge exposes atilla-ai's `completeSimple` as compaction's
/// `complete_simple`: it returns the faux provider's final message.
#[test]
fn complete_simple_bridges_to_atilla_ai() {
...
fn generate_summary_runs_through_the_bridge() {
    ...
    let summary = generate_summary(
        ...
        &models, // the atilla-ai wrapper, via the new trait impl
        &model,
...
/// than a panic: the bridge forwards atilla-ai's error stream.
#[test]
