...
//! `adaptOAuth` (`packages/coding-agent/src/core/provider-composer.ts:230`) and
...
//! `interaction.notify` / `interaction.prompt`. atilla-ai's [`OAuthAuth`] is not
...
//! (atilla-extensions), not here. This module defines the Rust callback surface
...
/// `extension-oauth-types.ts:11-14`). Maps to [`AuthEvent::AuthUrl`].
...
/// `extension-oauth-types.ts:17-22`). Maps to [`AuthEvent::DeviceCode`].
...
/// `extension-oauth-types.ts:4-8`). Maps to an [`AuthPromptKind::Text`] prompt.
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// `extension-oauth-types.ts:29-32`). Maps to an [`AuthPromptKind::Select`].
#[derive(Debug, Clone, PartialEq, Eq)]
...
///
/// The extension plane (atilla-extensions) implements this over its JS
/// `login(callbacks)` / `refreshToken` / `getApiKey` closures;
/// [`adapt_extension_oauth`] drives it through the thread + channel bridge.
