...
// atilla-coding's `provider_composer.rs`) also uses. The clone detector reads
...
//! operate on atilla-ai's own rich auth traits ([`ApiKeyAuth`], [`OAuthAuth`],
//! [`ProviderAuth`], [`ModelAuth`], [`AuthResult`]) and on [`Provider`]/
//! [`RegistryProvider`], so the AUTH layer is the additive rich surface that
//! belongs here in atilla-ai. The consumer crate `atilla-coding`
...
//!
//! atilla-coding's `core/provider_composer.rs` (PR #119) already ported the
...
//! resolver lives in `atilla-coding`, which atilla-ai cannot depend on. The
//! composers therefore take a [`ConfigValueResolver`] seam; `atilla-coding` wires
...
//! # Streaming
//!
//! atilla-ai's stream seam ([`RegistryProvider::stream`] / the api-registry
...
/// `!command`. `atilla-coding` implements this over its ported resolver; the
/// error-coupled command form lives on the far side of this seam so atilla-ai
...
///
/// `atilla-coding` adapts its richer `ModelsJsonProvider` into this at the call
...
impl ApiKeyAuth for ComposedApiKeyAuth {
    ...
    fn check(
        ...
        // (`if (inherited?.check) return inherited.check(input)`). atilla-ai's
...
/// dispatch. This is atilla-ai's counterpart to pi's composed `Provider`; the
/// credential-blind identity/model assembly is done by
/// `atilla-coding::core::provider_composer` (PR #119) and passed in.
...
/// from the credential-blind half (`atilla-coding::core::provider_composer`, PR
