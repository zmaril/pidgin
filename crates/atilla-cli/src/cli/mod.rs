...
pub fn run(argv: &[String]) -> i32 {
    ...
    if let Some(export) = &parsed.export {
        err_line(&format!(
            "Error: session HTML export is not yet implemented in atilla ({export})"
        ));
        return 1;
    }
    ...
    match app_mode {
        ...
        AppMode::Interactive => {
            err_line("Error: interactive mode is not yet implemented in atilla");
            1
        }
...
fn create_session_manager(
    ...
    if let Some(fork_arg) = &parsed.fork {
        ...
        return match resolve_session_path(fork_arg, cwd, session_dir) {
            ResolvedSession::Path(_) | ResolvedSession::Local(_) => {
                ...
                err_line("Error: --fork is not yet implemented in atilla");
                Err(1)
            }
    ...
    if parsed.resume {
        err_line("Error: --resume is not yet implemented in atilla");
        return Err(1);
    }
