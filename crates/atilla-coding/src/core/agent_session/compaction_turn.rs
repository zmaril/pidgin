...
impl AgentSession {
    ...
    pub(super) fn compaction_plan(
        ...
        if skip_aborted_check && assistant.stop_reason == atilla_ai::StopReason::Aborted {
            return CompactionPlan::None;
        }
        ...
        if same_model && is_context_overflow(&assistant, Some(context_window)) {
            let will_retry = assistant.stop_reason != atilla_ai::StopReason::Stop;
            if !will_retry {
        ...
        let context_tokens = if assistant.stop_reason == atilla_ai::StopReason::Error
