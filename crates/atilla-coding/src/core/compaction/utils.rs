...
// (crates/atilla-agent/src/harness/compaction/utils.rs) is intentional.

//! Shared utilities for compaction and branch summarization, mirroring
//! `packages/coding-agent/src/core/compaction/utils.ts`.
//!
//! [`AgentMessage`] is opaque JSON in this port, so the message-shape reads here
//! (`role`, `content`, tool-call blocks) go through [`serde_json::Value`]
//! accessors rather than typed fields, matching how the session-manager handles
//! agent messages.
//!
//! # `convertToLlm` note
//!
//! pi's coding-agent keeps `convertToLlm` (and its `bashExecutionToText` helper
//! plus the summary prefix/suffix constants) in `packages/coding-agent/src/core/
//! messages.ts`, not in `compaction/utils.ts`. atilla-coding has not ported that
//! `messages` module yet, and the summarization paths depend on `convertToLlm`,
//! so it lives here as a crate-internal helper for now, mirroring how the
//! agent-core compaction copy stages the same function. It should later move to
//! a shared `atilla_coding::core::messages` module.
...
use crate::core::session_manager::AgentMessage;
...
///
/// Extension seam: pi guards this on `!fromHook` so extension-authored (hook)
/// summaries do not double-count file operations that the extension engine
/// already tracks. atilla-coding has no extension engine yet; the guard is
/// honored by the callers via the `from_hook` field on the entry types.
...
/// compaction and branch-summarization summary generators, and exposed so a
/// future napi side can perform the identical text extraction.
                    ...
                    for block in blocks {
                        match block.get("type").and_then(Value::as_str) {
                            ...
                            Some("toolCall") => {
                                ...
                                // Divergence from the agent-core copy: pi's
                                // coding-agent uses bare `JSON.stringify(v)` here,
                                // not `safeJsonStringify`. `serde_json::to_string`
                                // on a `Value` is the closest equivalent.
                                let args_str =
...
/// Serialize a JSON value like JavaScript's bare `JSON.stringify(v)`, falling
/// back to the empty string. Divergence from the agent-core compaction copy,
/// which uses a `safeJsonStringify` wrapper; pi's coding-agent copy uses bare
/// `JSON.stringify`, so this port matches that observable output.
...
/// pi's `SUMMARIZATION_SYSTEM_PROMPT` (`utils.ts`).
pub const SUMMARIZATION_SYSTEM_PROMPT: &str = "You are a context summarization assistant. Your task is to read a conversation between a user and an AI assistant, then produce a structured summary following the exact format specified.

Do NOT continue the conversation. Do NOT respond to any questions in the conversation. ONLY output the structured summary.";

// ---------------------------------------------------------------------------
// convertToLlm and its constants (`packages/coding-agent/src/core/messages.ts`).
//
// Not part of pi's compaction/utils.ts; staged here as a crate-internal helper
// until atilla-coding ports a shared `messages` module. Mirrors pi's
// `convertToLlm`, `bashExecutionToText`, and the summary prefix/suffix constants
// byte-for-byte.
// ---------------------------------------------------------------------------
