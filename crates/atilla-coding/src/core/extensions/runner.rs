...
//! two-party contract the extension-plane owner implements from `atilla-extensions`
//! (deno-backed); atilla-coding ships only the trait and the always-compiled
//! stub, and **never names an atilla-extensions type** (the crate dependency runs
//! `atilla-extensions -> atilla-coding` only). There is deliberately **no**
//! `AgentSession` struct and **no** real impl here — the deno-backed
//! `RealExtensionRunner` lives in `atilla-extensions` behind its `deno` feature
...
/// `atilla-cli/args.rs` (a different surface with different variants).
...
pub trait SessionControlHost: Send + Sync {
    ...
    /// pi `getThinkingLevel`.
    fn get_thinking_level(&self) -> atilla_agent::types::ThinkingLevel;
    /// pi `setThinkingLevel`.
    fn set_thinking_level(&self, level: atilla_agent::types::ThinkingLevel);
}
...
pub trait SessionContextHost: Send + Sync {
    ...
    /// pi `getSignal`.
    fn get_signal(&self) -> atilla_ai::seams::AbortSignal;
    /// pi `abort`.
    fn abort(&self);
...
/// [`ExtensionLoader`](super::loader::ExtensionLoader) seam: atilla-coding defines
/// it and ships a no-op [`StubExtensionRunner`], the deno host in
/// `atilla-extensions` provides the real impl, the CLI injects it.
///
