...
// as `atilla-ai`'s `types.rs` does for the same reason.
...
//!
//! Structs reuse `atilla_ai::types` for the unambiguous fields (`ModelCost`,
//! `ModelCostTier`, `Modality`, `ThinkingLevelMap`).
//
// NOTE: the per-model/provider `compat` field is a union of the three
// `atilla_ai::types` `*Compat` structs (`OpenAICompletionsCompat`,
