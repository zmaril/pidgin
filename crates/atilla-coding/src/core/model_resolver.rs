...
//! objects.
//!
//! The model type is standardized on `atilla_ai::types::Model<serde_json::Value>`
//! (the crate's default compat view). pi's `ThinkingLevel` (`off | minimal |
//! low | medium | high | xhigh | max`) maps onto
//! [`atilla_ai::types::ModelThinkingLevel`], which carries the same seven
//! variants including `off`.

use atilla_ai::types::{Model, ModelThinkingLevel};
