...
//! `createModels({ modelsStore })` and the remote-catalog overlay. atilla-ai
//! has not ported that interface (there is no `trait ModelsStore` in
//! `crates/atilla-ai`), so this module defines the seam locally, matching
//! pi-ai's shape. It is **synchronous**, like the rest of atilla's storage
//! layer (pi's is `Promise`-returning). When atilla-ai lands a canonical
...
///
/// The seam is defined locally in atilla-coding because atilla-ai has not
