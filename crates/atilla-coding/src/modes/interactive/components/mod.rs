...
//! and *compose* the already-ported `atilla-tui` primitives (`Markdown`,
//! `Container`, `Box`, `Text`, `Spacer`) — none of those primitives are re-ported
//! here.
//!
//! Correctness is verified the same way the `atilla-tui` widget ports are: a
//! generator (`crates/atilla-coding/vectors/gen/generate_interactive_messages.mjs`)
//! drives pi's OWN component classes over representative inputs and dumps
//! `input -> rendered string[]` JSON; the Rust replay
//! (`crates/atilla-coding/tests/interactive_message_vectors.rs`) constructs the
...
use atilla_tui::markdown::{Markdown, MarkdownTheme, StyleFn};
use atilla_tui::renderer::Component;

use crate::modes::interactive::theme::Theme;

/// Minimal [`Component`] adapter over `atilla-tui`'s standalone [`Markdown`]
/// renderer. `atilla-tui` exposes `Markdown` as a pure `render(width)` renderer
/// rather than a `Component` (see the napi shim note in `atilla-napi`), so this
