...
```rust
let catalog = atilla_model_catalog::catalog();
...
## Intended atilla-ai integration

atilla-ai's provider registry consumes `catalog()` to enumerate providers and
...
calls of its own. This crate owns only the data and its parsing, while atilla-ai
...
atilla-ai can strongly-type `compat` per-API when it needs to.

## Data location decision

The snapshot lives in **`crates/atilla-model-catalog/data/`**, not in
