...
// ([`atilla_tui::CombinedAutocompleteProvider`]) abstracts those behind a
// [`atilla_tui::FileProvider`] trait; the pure prefix-extraction / ranking /
...
/// A native [`FileProvider`](atilla_tui::FileProvider) over `std::fs` /
...
impl atilla_tui::FileProvider for HostFileProvider {
    fn read_dir(&self, dir: &str) -> Result<Vec<atilla_tui::DirEntry>, atilla_tui::ProviderError> {
        // `readdirSync(dir, { withFileTypes: true })`. On Unix `file_type()` uses
        // the dirent type (no symlink follow), matching Node's `Dirent`.
        let read = std::fs::read_dir(dir).map_err(|_| atilla_tui::ProviderError)?;
        let mut out = Vec::new();
        for entry in read {
            let entry = entry.map_err(|_| atilla_tui::ProviderError)?;
            let name = entry.file_name().to_string_lossy().into_owned();
            let ft = entry.file_type().map_err(|_| atilla_tui::ProviderError)?;
            out.push(atilla_tui::DirEntry {
                name,
    ...
    fn stat_is_directory(&self, path: &str) -> Result<bool, atilla_tui::ProviderError> {
        // `statSync(path).isDirectory()` follows symlinks; `std::fs::metadata` does too.
        let md = std::fs::metadata(path).map_err(|_| atilla_tui::ProviderError)?;
        Ok(md.is_dir())
    }
    ...
    fn run_fd(&self, args: &[String]) -> atilla_tui::FdOutput {
        ...
        let Some(fd_path) = self.fd_path.as_ref() else {
            return atilla_tui::FdOutput {
                stdout: String::new(),
                code: None,
            };
        };
        match std::process::Command::new(fd_path).args(args).output() {
            Ok(output) => atilla_tui::FdOutput {
                stdout: String::from_utf8_lossy(&output.stdout).into_owned(),
                code: output.status.code(),
            },
            Err(_) => atilla_tui::FdOutput {
                stdout: String::new(),
                code: None,
            },
        }
...
fn commands_from_json(commands_json: &str) -> napi::Result<Vec<atilla_tui::Command>> {
    let parsed: Vec<CommandInput> = serde_json::from_str(commands_json)
        .map_err(|e| napi::Error::from_reason(format!("invalid commands: {e}")))?;
    Ok(parsed
        .into_iter()
        .map(|c| match c.name {
            Some(name) => atilla_tui::Command::Slash(atilla_tui::SlashCommand {
                name,
                description: c.description,
                argument_hint: c.argument_hint,
                get_argument_completions: None,
            }),
            None => atilla_tui::Command::Item(atilla_tui::AutocompleteItem {
...
fn suggestions_to_json(s: &atilla_tui::AutocompleteSuggestions) -> serde_json::Value {
    serde_json::json!({
...
pub struct AutocompleteCore {
    inner: atilla_tui::CombinedAutocompleteProvider<HostFileProvider>,
}
...
impl AutocompleteCore {
    ...
    pub fn new(
        ...
        Ok(Self {
            inner: atilla_tui::CombinedAutocompleteProvider::new(
    ...
    pub fn apply_completion_json(
        ...
        let item = atilla_tui::AutocompleteItem {
            value: item_in.value,
            label: item_in.label,
