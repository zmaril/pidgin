...
//
// A stateful `#[napi]` class wrapping `atilla_tui::Input`. The hand-written
...
impl InputCore {
    ...
    pub fn new() -> Self {
        ...
        let mut inner = atilla_tui::Input::new();
        {
            let ev = events.clone();
...
//
// A stateful `#[napi]` class wrapping `atilla_tui::SelectList`. pi's `render`
...
fn identity_select_theme() -> atilla_tui::SelectListTheme {
    atilla_tui::SelectListTheme {
...
impl SelectListCore {
    ...
    pub fn new(
        ...
        let items: Vec<atilla_tui::SelectItem> = items_in
            .into_iter()
            .map(|i| atilla_tui::SelectItem {
                value: i.value,
                label: i.label,
                description: i.description,
            })
            .collect();
        let layout = atilla_tui::SelectListLayoutOptions {
            min_primary_column_width,
            max_primary_column_width,
            truncate_primary: None,
        };
        Ok(Self {
            inner: atilla_tui::SelectList::new(items, max_visible, identity_select_theme(), layout),
        })
    }
