...
//! atilla-coding gains a live agent runtime that emits session events. Until
...
//!   module. atilla ships a single compiled binary, so both branches launch
//!   `atilla --mode rpc` — the Bun branch as a sibling of the current
//!   executable, the non-Bun branch as the current executable itself.
//! * pi generates request ids with `crypto.randomUUID` (v4). atilla has no
//!   `uuid` dependency; request ids use `atilla_agent::harness::session::uuidv7`
//!   (v7), matching how the rest of atilla mints ids. Only the id *format*
...
/// The coding-agent binary file name spawned in RPC mode (the atilla analog of
...
/// interpreter with the resolved `rpc-entry` module. atilla is always a compiled
/// binary, so both branches launch `atilla --mode rpc`: as a sibling of the
...
mod tests {
    ...
    fn spawn_command_bun_branch_uses_sibling_agent_binary() {
        let exe = Path::new("/opt/atilla/bin/orchestrator");
        let spawn = resolve_spawn_command(exe, true);
        assert_eq!(
            spawn.command,
            PathBuf::from("/opt/atilla/bin").join(agent_binary_file_name())
        );
    ...
    fn spawn_command_non_bun_branch_uses_current_exe() {
        let exe = Path::new("/usr/local/bin/atilla");
        let spawn = resolve_spawn_command(exe, false);
        assert_eq!(spawn.command, PathBuf::from("/usr/local/bin/atilla"));
        assert_eq!(spawn.args, vec!["--mode".to_string(), "rpc".to_string()]);
    }
