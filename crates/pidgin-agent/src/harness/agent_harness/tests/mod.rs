...
use pidgin_ai::seams::{AbortSignal, Provider};
use pidgin_ai::{AssistantMessage, Context, Message, Model, StopReason, StreamOptions};
...
fn user_texts(user: &pidgin_ai::UserMessage) -> Vec<String> {
    match &user.content {
        pidgin_ai::UserContent::Text(text) => vec![text.clone()],
        pidgin_ai::UserContent::Blocks(blocks) => blocks
            .iter()
    ...
    AgentTool {
        ...
        execute: Arc::new(|_id, args, _signal, _cb| {
            let expression = args
                ...
                .unwrap_or_default();
            AgentToolResult {
                content: vec![pidgin_ai::ContentBlock::Text {
