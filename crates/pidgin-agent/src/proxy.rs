...
//!
//! Per the crate convention (see [`crate::types`]), pidgin is
...
//! (`{ events, message }`) — the same eager shape every pidgin provider seam
//! converges on ([`pidgin_ai::seams::StreamResult`]). [`ProxyPartial::process_proxy_event`]
...
//! exact idiom pidgin-ai's own wire parsers use (see
//! `pidgin_ai::api::anthropic`'s `render_partial`).
//!
...
//! `toolcall_end`. pidgin-ai's [`ContentBlock::ToolCall`] has no such field, so
...
//! [`pidgin_ai::utils::json_parse::parse_streaming_json`], pidgin-ai's port of
...
use pidgin_ai::seams::clock::Clock;
use pidgin_ai::seams::provider::{AbortSignal, StreamResult};
use pidgin_ai::utils::json_parse::parse_streaming_json;
use pidgin_ai::{
