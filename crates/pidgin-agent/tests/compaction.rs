...
//! `createModels()` + `fauxProvider()` (pidgin-ai does not yet wrap `Models`);
...
use pidgin_ai::{AssistantMessage, Context, Model, StopReason, Usage};
...
use pidgin_agent::harness::session::{build_session_context, SessionContextBuildOptions};
...
fn serializes_conversation_with_truncated_tool_results() {
    ...
    let result = pidgin_agent::harness::compaction::serialize_conversation(&messages);
...
//
// pi ships no branch-summarization test file, so these are pidgin-authored
