...
//!   ~30ms into a slow (`tokensPerSecond: 20`) stream. pidgin's provider seam is
...
use pidgin_ai::{ContentBlock, Message, Model, StopReason, UserContent};

use pidgin_agent::agent::{Agent, AgentError, AgentOptions, InitialAgentState, Listener};
use pidgin_agent::types::{
    AgentEvent, AgentToolResult, AgentToolUpdateCallback, StreamFn, ThinkingLevel,
};
use pidgin_agent::{AgentTool, AgentToolExecute};
