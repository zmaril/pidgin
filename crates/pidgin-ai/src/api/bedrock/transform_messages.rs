...
// duplication, but they are distinct, load-bearing transforms kept verbatim.
...
//!
//! DE-DUP / REBASE POINT: this is a **driver-local** copy, identical to
//! [`crate::api::mistral`]'s copy. `transform-messages.ts` is a shared pi helper
//! (used by the Mistral, Google, and Bedrock dialects) and belongs under a
...
/// (Bedrock passes its `[a-zA-Z0-9_-]` sanitizer). `timestamp` seeds the
/// synthetic tool-result messages pi stamps with `Date.now()`; the value is never
/// emitted on the wire.
