...
// error-body surfacing mirror the direct-Gemini driver by design (pi keeps the
// two Google drivers as near-duplicate copies diverging only in client/auth
// construction); the clone detector reads the shared boundary-type construction
// and the send/`format_api_error` ladder as duplicative. Each is a distinct,
// load-bearing transcription kept parallel to the google-generative-ai driver on
// purpose.
//! The Google Vertex AI stream driver, the transport-driving half of pi-ai's
//! `google-vertex.ts` `stream` at pinned commit `3da591ab`.
//!
//! [`stream`] is the buffered, seam-targeted analogue of pi's async `stream()`:
//! it resolves the credential, builds the request body
...
//! [`GoogleVertexSseDecoder`](super::GoogleVertexSseDecoder) the buffered path
//! uses — one source of truth for the event sequence.
//!
//! # Auth scope
//!
...
//! event with no preceding `start`. The buffered driver reproduces that: an
//! unresolvable API key yields `No API key for provider: <provider>`;
//! `build_params` returning `Err` (a pre-aborted signal) surfaces its message; a
//! non-2xx create carries the API's diagnostic through [`format_api_error`]
//! rather than discarding the body.
...
/// occurs before the stream's `start` event.
...
/// Vertex Express credential ([`super::resolve_api_key`], applied by the backend);
/// `options_headers` are the per-request header overrides; `request_options`
/// carries the request-shaping subset used by [`build_params`]. `timestamp` is
/// the message timestamp pi sets via `Date.now()` (threaded here for determinism,
/// as the decoder already is).
pub fn stream<T: HttpTransport + ?Sized>(
    ...
    match transport.send(&request) {
        Ok(response) if response.is_ok() => {
            ...
            // `GoogleVertexSseDecoder` the incremental path uses, so the buffered
            // events + terminal message are byte-identical.
...
/// [`GoogleVertexSseDecoder`] the buffered path uses -- one source of truth for
/// the event sequence. A pre-stream failure (missing credential, a pre-aborted
...
pub fn stream_streaming<'a, T: HttpTransport + ?Sized>(
    ...
    match transport.send_streaming(&request) {
        Ok(response) if (200..300).contains(&response.status) => {
            let decoder = GoogleVertexSseDecoder::new(model.clone(), timestamp);
