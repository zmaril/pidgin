...
//! # Location
//!
//! The seams live in `pidgin-ai`, the leaf crate in the workspace dependency
//! graph (`ai ──▶ agent ──▶ coding-agent`). Placing them at the leaf lets the
//! faux provider (in `pidgin-ai`) implement [`provider::Provider`] without a
//! dependency cycle, while the `pidgin-core` façade re-exports this module so the
