...
mod tests {
    ...
    fn empty_scoped_value_falls_through_to_process_env() {
        std::env::set_var("PIDGIN_TEST_EMPTY_FALLTHROUGH", "from-process");
        let env = env_of(&[("PIDGIN_TEST_EMPTY_FALLTHROUGH", "")]);
        assert_eq!(
            get_provider_env_value("PIDGIN_TEST_EMPTY_FALLTHROUGH", Some(&env)),
            Some("from-process".to_string())
        );
        std::env::remove_var("PIDGIN_TEST_EMPTY_FALLTHROUGH");
    }

    #[test]
    fn reads_process_env_when_no_override() {
        std::env::set_var("PIDGIN_TEST_PROCESS_ONLY", "value");
        assert_eq!(
            get_provider_env_value("PIDGIN_TEST_PROCESS_ONLY", None),
            Some("value".to_string())
        );
        std::env::remove_var("PIDGIN_TEST_PROCESS_ONLY");
    }

    #[test]
    fn missing_everywhere_is_none() {
        assert_eq!(
            get_provider_env_value("PIDGIN_TEST_DEFINITELY_UNSET_KEY", None),
            None
        );
    }
    ...
    fn empty_process_env_value_is_none() {
        std::env::set_var("PIDGIN_TEST_EMPTY_PROCESS", "");
        assert_eq!(
            get_provider_env_value("PIDGIN_TEST_EMPTY_PROCESS", None),
            None
        );
        std::env::remove_var("PIDGIN_TEST_EMPTY_PROCESS");
    }
}
