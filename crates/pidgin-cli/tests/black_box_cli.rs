...
//! `session-file-invalid`). Each test spawns the built `pidgin` binary and
//! asserts the exact stdout / stderr / exit-code / on-disk behavior that pi's
//! vitest suites assert, so the same 15 cases pass against `pidgin`.
...
fn pidgin_bin() -> &'static str {
    env!("CARGO_BIN_EXE_pidgin")
}
...
fn unique_temp_dir(tag: &str) -> PathBuf {
    ...
    let nanos = std::time::SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("pidgin-cli-{tag}-{pid}-{n}-{nanos}"));
...
/// Spawn the pidgin binary. `env` is a set of extra environment overrides.
fn run_cli(args: &[&str], cwd: &Path, env: &[(&str, &str)]) -> Output {
    let mut cmd = Command::new(pidgin_bin());
    ...
    let out = cmd.output().expect("failed to spawn pidgin binary");
    Output {
