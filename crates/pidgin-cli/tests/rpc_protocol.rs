...
//!
//! Spawns the `pidgin` binary in `--mode rpc`, feeds JSONL commands over stdin,
//! and asserts the framed responses. This is an pidgin-owned test (pi's own
...
fn run_rpc(commands: &[&str]) -> Vec<Value> {
    let exe = env!("CARGO_BIN_EXE_pidgin");
    let session_dir = std::env::temp_dir().join(format!("pidgin-rpc-test-{}", std::process::id()));
