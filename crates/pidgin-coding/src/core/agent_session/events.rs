...
use pidgin_agent::types::{AgentEvent, AgentMessage, ThinkingLevel};
use pidgin_ai::{AssistantMessageEvent, ToolResultMessage};
...
/// verbatim so the wire tag/field bytes match [`pidgin_agent::AgentEvent`]) plus
/// the ten session-specific variants, including the session's own `agent_end`
/// that overrides the core one by adding `willRetry`.
///
/// Internally tagged exactly like [`pidgin_agent::AgentEvent`]: snake_case `type`
...
///
/// pi's listener is a synchronous `(event) => void`. pidgin's TUI drives the
