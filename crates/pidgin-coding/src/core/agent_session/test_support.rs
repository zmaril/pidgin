...
//!
//! The harness wires a faux [`StreamFn`](pidgin_agent::types::StreamFn) driven by
...
//!
//! `pidgin_agent`'s agent loop is synchronous and eager: a `prompt` runs the
...
use pidgin_agent::types::{AgentMessage, AgentTool, AgentToolResult, AgentToolUpdateCallback};
