...
//!   [`AgentEvent`](pidgin_agent::types::AgentEvent) to its extension event, fan
...
use pidgin_agent::agent::{AgentError, Listener};
use pidgin_agent::types::{AgentEvent, AgentMessage};
...
/// The `"unknown"` placeholder [`Model`] id/provider `pidgin_agent`'s
/// [`AgentState`](pidgin_agent::types::AgentState) uses where pi represents an
/// unselected model as `agent.state.model === undefined`. `pidgin_agent`'s field
...
/// streaming guard) or an error surfaced by the wrapped [`Agent`](pidgin_agent::agent::Agent).
///
...
pub(super) struct AgentEventHandler {
    ...
    pub agent: pidgin_agent::agent::Agent,
...
impl AgentSession {
    ...
    ///
    /// `pidgin_agent`'s `AgentState.model` is non-optional and defaults to the
