...
//! pidgin-ai ports the `Provider`/faux surface but not yet a `Models`/
...
use pidgin_ai::{AssistantMessage, Context, Message, Model, StopReason, Usage};
...
/// "extension-specific data (e.g. ArtifactIndex)". pidgin-coding has no
...
fn extract_file_operations(
    ...
    if prev_compaction_index >= 0 {
        if let SessionEntry::Compaction(prev) = &entries[prev_compaction_index as usize] {
            ...
            // extension-authored (hook) compactions are not re-counted. pidgin
