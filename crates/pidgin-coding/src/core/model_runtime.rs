...
//! credential-aware read/refresh half is wired against pidgin-ai's landed [`Models`]
...
//! inside pidgin-ai (its own residual deferral): the [`RegistryProvider`] the
...
//!   (`compose_rich_provider`) that composes the rich [`ProviderAuth`](pidgin_ai::auth::ProviderAuth)
...
//!   `radiusProvider({ id, name, gateway })`): pidgin-ai's [`radius_provider`]
...
impl ModelRuntime {
    ...
    fn recompose_provider(&mut self, provider_id: &str) {
        ...
        // AUTH half (pidgin-ai) then composes the rich `ProviderAuth` handlers.
    ...
    ///
    /// The env-key/ambient auth subset pidgin-ai's [`Models::check_auth`] resolves
    ...
    /// composing the rich [`ProviderAuth`](pidgin_ai::auth::ProviderAuth) handlers
    ...
    /// [`ProviderAuth`](pidgin_ai::providers::registry::ProviderAuth) env-key
    ...
    /// deferred inside pidgin-ai (see the module docs).
