...
use pidgin_ai::auth::AuthResolutionOverrides;
use pidgin_ai::get_supported_thinking_levels;
use pidgin_ai::providers::registry::{
    create_provider, ApiRouting, CreateProviderOptions, ProviderAuth, RegistryProvider,
};
use pidgin_ai::types::{Modality, Model, ModelCost, ModelThinkingLevel};
...
fn get_api_key_and_headers_resolves_env_key() {
    let _guard = ENV_TEST_LOCK.lock().unwrap();
    std::env::set_var("PIDGIN_RT_APIKEY_98765", "resolved-secret");

    let mut fx = fixture(json!({}));
    fx.registry
        .register_native_provider(native_provider_with_auth(
            "env-native",
            ProviderAuth::env_api_key("Env key", &["PIDGIN_RT_APIKEY_98765"]),
        ))
        .unwrap();
    ...
    assert_eq!(auth.source.as_deref(), Some("PIDGIN_RT_APIKEY_98765"));
    ...
    std::env::remove_var("PIDGIN_RT_APIKEY_98765");
}
...
fn env_check_auth_drives_provider_auth_status() {
    let _guard = ENV_TEST_LOCK.lock().unwrap();
    std::env::set_var("PIDGIN_RT_ENVCHK_98765", "env-value");
    ...
    fx.registry
        .register_native_provider(native_provider_with_auth(
            "env-status",
            ProviderAuth::env_api_key("Env key", &["PIDGIN_RT_ENVCHK_98765"]),
        ))
        .unwrap();

    // checkAuth resolves the env-var source.
    let check = fx.runtime().check_auth("env-status").unwrap().unwrap();
    assert_eq!(check.source.as_deref(), Some("PIDGIN_RT_ENVCHK_98765"));
    ...
    assert!(status.configured);
    assert_eq!(status.source, Some(AuthSource::Environment));
    assert_eq!(status.label.as_deref(), Some("PIDGIN_RT_ENVCHK_98765"));
    ...
    std::env::remove_var("PIDGIN_RT_ENVCHK_98765");
