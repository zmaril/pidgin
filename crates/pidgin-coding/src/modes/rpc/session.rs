...
//! the storage-level session tree (`pidgin_agent::harness::session::Session`),
...
use pidgin_agent::harness::types::SessionTreeEntry;
...
impl RpcSession {
    ...
    pub fn export_html(&self, output_path: Option<&str>) -> Result<Value, String> {
        ...
        let path = match output_path {
            ...
            None => std::env::temp_dir().join(format!("pidgin-session-{}.html", self.session_id)),
        };
