...
//! have MOVED to `crates/pidgin-extensions/tests/deno_resource_loader.rs`
...
//! the real loader is in pidgin-extensions (which depends on pidgin-coding and
//! needs V8), so referencing it from an pidgin-coding test would be a dependency
...
use pidgin_coding::core::resource_loader::ExtensionPathEntry;
...
use pidgin_coding::core::settings_manager::{SettingsManager, SettingsManagerCreateOptions};
use pidgin_coding::core::source_info::{PathMetadata, SourceOrigin, SourceScope};
...
fn apply_skills_override() {
    let e = env();
    let injected = pidgin_coding::core::skills::Skill {
        name: "injected".to_string(),
        description: "Injected skill".to_string(),
        file_path: "/fake/path".to_string(),
        base_dir: "/fake".to_string(),
        source_info: pidgin_coding::core::skills::SourceInfo {
            path: "/fake/path".to_string(),
            source: "custom".to_string(),
            scope: pidgin_coding::core::skills::SourceScope::Temporary,
            origin: pidgin_coding::core::skills::SourceOrigin::TopLevel,
            base_dir: None,
        },
    ...
    options.skills_override = Some(Box::new(move |_base| {
        pidgin_coding::core::skills::LoadSkillsResult {
...
// `crates/pidgin-extensions/tests/deno_resource_loader.rs` (deno-gated), where
// the real `RealExtensionLoader` is injected via the seam. They can't live here:
// the real loader is in pidgin-extensions (depends on pidgin-coding + needs V8),
// so referencing it from an pidgin-coding test would be a dependency cycle and
