...
//! event name in `globalThis.__pidgin.registry.hooks` (a JS-side `Map<event,
...
//! `globalThis.__pidgin.invokeHook` bootstrap function (see [`crate::api_ops`]),
...
/// The plain-data envelope `globalThis.__pidgin.invokeHook` returns (as a
...
pub async fn invoke_hook_on_runtime(
    ...
    let arg_list = args
        ...
        .join(", ");
    let code = format!("globalThis.__pidgin.invokeHook({arg_list})");
...
/// The plain-data envelope `globalThis.__pidgin.invokeStored` returns (as a
...
pub async fn invoke_stored_on_runtime(
    ...
    let arg_list = call_args
        ...
        .map_err(|e| anyhow!("encode call args: {e}"))?
        .join(", ");
    let code = format!("globalThis.__pidgin.invokeStored({arg_list})");
