...
//! pidgin-coding's [`Extension`].
//!
//! # Dependency inversion (why this lives in pidgin-extensions)
//!
//! `pidgin-extensions` depends on `pidgin-coding` (for the seam types, the
//! discovery records, and the `ExtensionHost` registration surface); pidgin-coding
//! must **never** depend on pidgin-extensions. So the real `impl ExtensionLoader`
//! cannot live in pidgin-coding — it lives here and is injected into `reload()`
