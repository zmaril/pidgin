...
//! `clap` (the pidgin workspace's chosen arg parser), so the subcommand/flag
...
//! a `[[bin]]` named `pidgin-orchestrator` at `src/main.rs`; this module holds the
...
use pidgin_orchestrator::config::{get_socket_path, VERSION};
use pidgin_orchestrator::ipc::client::send_ipc_request;
use pidgin_orchestrator::ipc::protocol::{
    encode_message, ListRequest, OrchestratorRequest, RpcRequest, RpcStreamRequest, SpawnRequest,
    StatusRequest, StopRequest,
};
use pidgin_orchestrator::serve::serve;

/// The `orchestrator` CLI (pi's `cli.ts`).
#[derive(Debug, Parser)]
#[command(
    name = "pidgin-orchestrator",
    version = VERSION,
...
mod tests {
    ...
    fn parse(args: &[&str]) -> Result<Cli, clap::Error> {
        let mut argv = vec!["pidgin-orchestrator"];
