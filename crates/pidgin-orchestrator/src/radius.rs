...
//! as pidgin-ai's OAuth flows:
//!
//! - **HTTP** goes through pidgin-ai's injected [`HttpTransport`] seam (the same
...
//!   through [`pidgin_coding::core::auth::read_stored_credential`] (pi's
...
use pidgin_ai::auth::Credential;
use pidgin_ai::seams::http::{HttpRequest, HttpResponse, HttpTransport};
...
/// read is a one-off lookup via [`pidgin_coding::core::auth::read_stored_credential`]
...
fn stored_radius_credential() -> Option<pidgin_ai::auth::OAuthCredential> {
    match pidgin_coding::core::auth::read_stored_credential(RADIUS_PROVIDER, None) {
