...
use pidgin_tui::{DefaultTextStyle, Markdown, MarkdownOptions, StyleFn};
...
fn markdown_render_vectors_are_byte_exact() {
    ...
    for case in &vectors.cases {
        let theme = pidgin_tui::default_markdown_theme();
...
fn markdown_render_matches_direct_construction() {
    ...
    let direct = Markdown::new(
        ...
        0,
        0,
        pidgin_tui::default_markdown_theme(),
        None,
        None,
    )
    .render(width);
    let wrapped = pidgin_tui::markdown_render(source, width);
...
fn markdown_render_matches_vector_for_h1() {
    ...
    {
        assert_eq!(
            pidgin_tui::markdown_render(&case.input, case.width),
            case.raw
        );
    }
}
