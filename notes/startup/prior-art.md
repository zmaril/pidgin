...
Note (2026-07-18): the survey stands, but two recommendations in section 4 were later overruled — providers are hand-rolled thin clients (rust-genai rejected) and the TUI is a faithful port of pi's renderer rather than a ratatui rebuild. The design doc (`notes/design.md`) records the current decisions.
